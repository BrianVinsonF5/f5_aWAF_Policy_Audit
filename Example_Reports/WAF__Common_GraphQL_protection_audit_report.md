# WAF Policy Compliance Report for `/Common/GraphQL_protection` on `ip-10-1-1-4.us-west-2.compute.internal` (10.1.1.4)

**Source Device:** `ip-10-1-1-4.us-west-2.compute.internal` (10.1.1.4)

## Policy: `/Common/GraphQL_protection`

- **Partition:** Common
- **Enforcement Mode:** transparent
- **Baseline Policy:** baseline.xml
- **Audit Date:** 2026-04-03T19:48:05Z
- **Compliance Score:** 60.0% — **FAIL** (threshold: 90%)

### Virtual Server Bindings

| Virtual Server | IP Address | Port | Association | Local Traffic Policies |
|----------------|:----------:|:----:|:-----------:|------------------------|
| `/Common/owasp-juiceshop_443_vs` | 10.1.10.145 | 443 | direct | `/Common/asm_auto_l7_policy__owasp-juiceshop_443_vs` |
| `/Common/dvga_443` | 10.1.10.147 | 80 | direct | `/Common/asm_auto_l7_policy__dvga_443` |
...
|---:|-----------|------|--------|
| 1 | — | — | Parent Policy was set to empty value.
...
Security Policy Description was set to GraphQL Policy.
Template was set to POLICY_TEMPLATE_GRAPHQL.
...
Policy Name was set to /Common/GraphQL_protection.
...
**Learning Mode:** `Manual` — **⚠ MANUAL** *(Baseline: `Automatic`)*

### Policy Builder Settings

| Section | Setting | Baseline | Target | Match |
|---------|---------|----------|--------|-------|
| Core | Learning Mode | Automatic | Manual | ⚠ |
...
| Core | Response Codes | 1xx, 2xx, 3xx | 1xx, 3xx, 2xx | ⚠ |
| Cookie | Learn Cookies | When Violation Detected | Never | ⚠ |
...
| Parameter | Learn Parameters | Compact | Never | ⚠ |
...
| Server Technologies | Learn Server Technologies | Enabled | Disabled | ⚠ |
...
|----------|----------|---------|------|-------|
| blocking | 0 | 14 | 0 | 14 |
| policy-builder | 0 | 4 | 4 | 8 |
| urls | 0 | 1 | 0 | 1 |
| **Totals** | **0** | **19** | **4** | **23** |

- **Missing elements (in baseline, absent in target):** 15
- **Extra elements (in target, not in baseline):** 21
...
- **Baseline:** present
- **This Policy:** missing
- **Impact:** urls '*' defined in baseline is missing from target.
...
### 8. blocking: HEADER_REPEATED
- **Attribute:** `(all)`
- **Baseline:** present
- **This Policy:** missing
- **Impact:** Blocking violation 'Illegal repeated header' (HEADER_REPEATED) is in baseline but absent from target.

### 9. blocking: EMPTY_PARAM_VALUE
- **Attribute:** `(all)`
- **Baseline:** present
- **This Policy:** missing
- **Impact:** Blocking violation 'Illegal empty parameter value' (EMPTY_PARAM_VALUE) is in baseline but absent from target.

### 10. blocking: PARAM_VALUE_NOT_MATCHING_REGEX
- **Attribute:** `(all)`
- **Baseline:** present
- **This Policy:** missing
...
### 14. blocking: WEBSOCKET_TEXT_MESSAGE_NOT_ALLOWED
- **Attribute:** `(all)`
- **Baseline:** present
- **This Policy:** missing
...
### 16. policy-builder: learningMode
- **Attribute:** `learningMode`
- **Baseline:** Automatic
- **This Policy:** Manual
- **Impact:** Policy Builder learning mode differs from baseline.

### 17. policy-builder.cookie: learnCookies
- **Attribute:** `learnCookies`
- **Baseline:** When Violation Detected
- **This Policy:** Never
- **Impact:** Policy Builder cookie 'learnCookies' differs from baseline.

### 18. policy-builder.parameter: learnParameters
- **Attribute:** `learnParameters`
- **Baseline:** Compact
- **This Policy:** Never
- **Impact:** Policy Builder parameter 'learnParameters' differs from baseline.

### 19. policy-builder.url: learnUrls
- **Attribute:** `learnUrls`
- **Baseline:** Compact
- **This Policy:** Never
...
### 4. policy-builder.serverTechnologies: learnServerTechnologies
- **Attribute:** `learnServerTechnologies`
- **Baseline:** Enabled
- **This Policy:** Disabled
- **Impact:** Policy Builder serverTechnologies 'learnServerTechnologies' differs from baseline.
...
| `HEADER_REPEATED` | HEADER_REPEATED | `(all)` | present | missing | ⚠ WARNING |
...
- `{'section': 'blocking', 'id': 'HEADER_REPEATED', 'name': 'Illegal repeated header'}`
